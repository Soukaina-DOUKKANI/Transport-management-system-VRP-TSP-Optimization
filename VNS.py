...
class Variable_neighborhood_search:
    ...
    def __init__(self, filepath, max_attempts=20, neighbourhood_size=5, iterations=100):
        ...
        #import distance matrix from excel file
        X=pd.read_excel(filepath)
        X = X.drop(X.columns[0], axis=1)
        #get values 
        X=X.values
        #import coordinates 
        dataset=pd.read_excel(r'C:\Users\soukaina\Desktop\TMS\files\tms.xlsx')
        #get cities  
        ...
        self.optimum_dist = round(optimum_temp[1])
    ...
    def distance_calc(self, Xdata, city_tour):
        ...
        for i in range(0, len(city_tour[0])-1):
            j = i + 1
            distance = distance + Xdata[city_tour[0][i]-1, city_tour[0][j]-1]
        return distance
    
    # create city_tour function
